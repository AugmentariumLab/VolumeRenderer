...
void main()
{ 
	vFragColor = vec4(255.0, 255.0, 255.0, 1.0);
	...
	for (int i = 0; i < MAX_SAMPLES; i++) {
		...
		if( (sample -isoValue) < 0  && (sample2-isoValue) >= 0.0)  {
			...
			//vFragColor =  PhongLighting(L,N,V,250,  vec3(tc.z, 0.5, 0.5));
			//vFragColor =  PhongLighting(L,N,V,250,  vec3(0.5, 0.5, tc.z));
			vFragColor =  PhongLighting(L,N,V,250,  vec3(0.39, 0.58, 0.93));
			break;
		} 
	} 
}